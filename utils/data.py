...
###get [train, val, test] data csv
def get_split(conf):
    if conf.dataset == 'kiba':
        split = get_kiba_split(conf)

    elif conf.dataset == 'kd':
        split = get_kd_split(conf)

    elif conf.dataset == 'dude':
        split = get_dude_split(conf)

    return split


def get_kiba_split(conf):
    ...
    contactmap_files = os.listdir(conf.contactmap_path)
...
def get_dude_split(conf):
    contactmap_files = os.listdir(conf.contactmap_path)

    split_names = ['train', 'test']
    df_train = pd.read_csv(conf.root_dir + 'my_data/dude/dude_train.csv')
    df_test = pd.read_csv(conf.root_dir + 'my_data/dude/dude_test.csv')
    split = {}
...
###delete data with smiles length over 77
def modify_datacsv(data_csv):
...
class tdcDataset(Dataset):
    ...
    def __init__(self, data_csv, conf):
    ...
    def __getitem__(self, index):
        ...
        ###contactmap, smiles, label, contactmap size
        return self.uniprotID_contactmap_dict[uniprotID], self.index_drug_map[index], \
               torch.tensor(int(label)).cuda().float(), self.uniprotID_size_dict[uniprotID]
...
def test(data_loader, model, conf):
    ...
    for batch_idx, (contactmap, input_text, properties, size) in enumerate(data_loader):
        ...
        output = model(input_text.cuda(), contactmap.cuda().float(), hidden_state)
        if conf.model_type == 'drugVQA':
            output = output[0]
...
def get_data_loader(split, conf):
    ...
    for i in conf.split_names:
        if conf.test_mode:
            sub_datacsv = split[i][0:400]
        else:
            sub_datacsv = split[i]
        sub_datacsv = modify_datacsv(sub_datacsv)
        sub_dataset = tdcDataset(data_csv=sub_datacsv, conf=conf)
